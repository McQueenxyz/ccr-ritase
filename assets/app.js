...
(function () {
  ...
  const VBOX = { dumptruck: "3000 -600 79000 38000" };
  const BRAND = { dumptruck: 1, whatsapp: 1, instagram: 1, x: 1, threads: 1, facebook: 1, linkedin: 1, youtube: 1, behance: 1 };
  function icon(name, size) {
    const inner = ICONS[name] || ""; const s = size || 20;
    const vb = VBOX[name]; // ikon berformat lebar (mis. dump truck 2:1) → lebar disesuaikan
    const w = vb ? Math.round(s * 1.55) : s;
    return `<svg class="ic${BRAND[name] ? " brand" : ""}" viewBox="${vb || "0 0 24 24"}" width="${w}" height="${s}" aria-hidden="true">${inner}</svg>`;
  }
